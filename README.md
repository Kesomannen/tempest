...
- Manage mods with a manifest-based profile format, inspired by Cargo and similar package managers.
- Install mods from multiple sources, including Thunderstore, Hexium, Github Releases and local files.
...
The Rust toolchain is required to build it yourself. To start, first clone the repository:
...
> [!NOTE]
> Unlike traditional mod managers like [r2modman]() and [Gale](), Tempest does not store mod profiles in a central directory. Instead it works similarly to tools like `git`; any directory with a valid `tempest.toml` is recognized as a profile.
...
Add a mod to the manifest and install it along with dependencies by using `add`:
...
> [!NOTE]
> You will see `BepInExPack` marked as "transitive". This means it was found and installed as a dependency and not mentioned in the manifest.
